 ...
 */
/**
 * OAuth 2.1 authorization server — ADAPTED from codex-with-chatgpt (MIT).
 *   upstream: src/auth/oauth.ts
 *   changes:  product naming generalized, pairing page copy rewritten for
 *             Agent2LLM, `sessionId` propagated into tokens, extra
 *             `.well-known` routes unchanged (RFC 8414 / RFC 7591 / RFC 7009).
...
import type { PairingManager } from "@agent2llm/pairing";
...
export interface OAuthDeps {
  ...
  productName: string;
  /** Bind issued tokens to a collaboration session, when one exists. */
  sessionId?: string;
...
export function protectedResourceMetadata(base: string, productName: string): Record<string, unknown> {
  return {
    resource: `${base}/mcp`,
...
const SCOPE_LABELS: Record<string, string> = {
  ...
  "execution.read": "Read execution summaries and released output",
  offline_access: "Stay connected between sessions",
};

export function pairingPage(opts: {
  requestId: string;
  workspaceName: string;
  productName: string;
  scopes: string[];
...
<div class="card">
  <h1>${product}</h1>
  <p class="sub">A Brain client is requesting read-only access to workspace <strong>${escapeHtml(opts.workspaceName)}</strong>:</p>
  <ul>${scopeList}</ul>
  ...
  <p class="hint">The pairing code was generated by Agent2LLM on this computer.<br>It expires in a few minutes.</p>
</div>
</body>
</html>`;
}
...
export function createOAuthRouter(deps: OAuthDeps): Router {
  ...
  const prMetadataHandler = (req: Request, res: Response): void => {
    res.json(protectedResourceMetadata(deps.getBaseUrl(req), deps.productName));
  };
  ...
  router.post("/oauth/register", json(), (req, res) => {
    ...
    const client = deps.store.registerClient({
      ...(typeof body.client_name === "string" ? { clientName: body.client_name.slice(0, 200) } : {}),
      redirectUris: redirectUris as string[],
    });
    deps.logger.info(`Registered OAuth client ${client.clientId}`);
    res.status(201).json({
  ...
  router.get("/oauth/authorize", (req, res) => {
    ...
    if (!client) {
      ...
      res.status(400).send("Unknown client. Please reconnect from your Brain client.");
      return;
    }
  ...
  router.post("/oauth/authorize", urlencoded({ extended: false }), (req, res) => {
    ...
    if (!request) {
      setAuthSecurityHeaders(res);
      res.status(400).send("This authorization request has expired. Please reconnect from your Brain client.");
      return;
    }
    ...
    if (!verdict.ok) {
      const messages: Record<string, string> = {
        ...
        expired: "This pairing code has expired. Ask Agent2LLM to generate a new one.",
        too_many_attempts: "Too many incorrect attempts. Ask Agent2LLM to generate a new pairing code.",
        rate_limited: "Too many attempts. Please wait a minute and try again.",
        no_active_session: "No active pairing session. Ask Agent2LLM to generate a pairing code.",
      };
  ...
  router.post("/oauth/token", urlencoded({ extended: false }), json(), (req, res) => {
    ...
    if (grantType === "refresh_token") {
      ...
      res.json({
        ...
        ...(result.tokens.refreshToken ? { refresh_token: result.tokens.refreshToken } : {}),
